...
<form action="index.php" method="post" name="adminForm" id="adminForm" enctype="multipart/form-data"
	...
	<input type="hidden" name="reddesign_background_id"
	       value="<?php echo $this->item->reddesign_background_id; ?>">
	<input type="hidden" name="<?php echo JFactory::getSession()->getFormToken(); ?>" value="1"/>

	<div id="basic_configuration" class="span12">
		<h3><?php echo JText::_('COM_REDDESIGN_TITLE_BACKGROUNDS_EDIT'); ?></h3>
		...
		<div class="control-group">
			...
			<div class="controls">
				<input type="text" name="title" id="title" value="<?php echo $this->item->title; ?>"
					...
					<span
						class="help-block"><?php echo JText::_('COM_REDDESIGN_BACKGROUND_FIELD_TITLE_DESC'); ?></span>
			</div>
		</div>
		<div class="control-group">
		...
		<?php if (!empty($this->item->epsfile) && !empty($this->item->jpegpreviewfile)) : ?>
			<h4><?php echo JText::_('COM_REDDESIGN_BACKGROUND_SUBTITLE_AREA'); ?></h4>
			<?php echo $this->loadTemplate('preview'); ?>
		<?php else : ?>
			<h4><?php echo JText::_('COM_REDDESIGN_BACKGROUND_SUBTITLE_UPLOAD'); ?></h4>
			<?php echo $this->loadTemplate('upload'); ?>
		<?php endif; ?>
		<h4><?php echo JText::_('COM_REDDESIGN_BACKGROUND_SUBTITLE_DESIGN'); ?></h4>
		<div class="control-group">
			<label class="control-label " for="reddesign_design_id">
				<?php echo JText::_('COM_REDDESIGN_BACKGROUND_FIELD_DESIGN'); ?>
			</label>
			...
			<div class="controls">
				...
				?>
				<span
					class="help-block"><?php echo JText::_('COM_REDDESIGN_BACKGROUND_FIELD_DESIGN_DESC'); ?></span>
			</div>
		</div>
		<h4><?php echo JText::_('COM_REDDESIGN_BACKGROUND_SUBTITLE_FONTS'); ?></h4>
		<?php if (empty($this->fonts_list)) : ?>
			<div class="alert">
				<button type="button" class="close" data-dismiss="alert">×</button>
				<strong><?php echo JText::_('COM_REDDESIGN_COMMON_WARNING'); ?></strong> <?php echo JText::_('COM_REDDESIGN_BACKGROUND_NO_FONTS'); ?>
			</div>
		<?php else : ?>
