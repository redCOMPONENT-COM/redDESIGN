 ...
 *
 * @original    author ZaraWebFX
 *              found at http://www.phpclasses.org/package/2144-PHP-Retrieve-details-from-TrueType-font-files.html
 *
 * @package     RedDesign.Component
 * @subpackage  Administrator
 *
...
class Ttf
{
	...
	public function get_friendly_ttf_name($ttf_filename)
	{
		...
		for ($j = 0; $j < $number_name_records_dec; $j++)
		{
			...
			if ($name_id_dec == 1 && !isset($fontfamily))
			{
				$fontfamily = '';
			...
			if ($name_id_dec == 2 && !isset($fontsubfamily))
			{
				$fontsubfamily = '';
			...
			if (!isset($fontfamily) && !isset($fontsubfamily) && !isset($fullfontname) && !isset($copyright))
			{
				break;
			}
		}
